    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="健身项目" prop="jianshenxiangmu">
          <el-select v-model="ruleForm.jianshenxiangmu" placeholder="请选择健身项目">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="健身器材" prop="jianshenqicai">
          <el-select v-model="ruleForm.jianshenqicai" placeholder="请选择健身器材">
            <el-option
                v-for="(item,index) in jianshenqicaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="健身器材" prop="jianshenqicai">
	      <el-input v-model="ruleForm.jianshenqicai"
                placeholder="健身器材" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  data() {
	...
	shouji : false,
      ...
      rules: {
          ...
          jianshenqicai: [
                { required: true, message: '健身器材不能为空', trigger: 'blur' },
          ],
          shijian: [
          ],
          huiyuanzhanghao: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jianshenxiangmu = true;
          ...
          if(o=='jianshenqicai'){
            this.ruleForm.jianshenqicai = obj[o];
	    this.ro.jianshenqicai = true;
            continue;
          }
          if(o=='shijian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jianshenxiangmu/xiangmumingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.jianshenqicaiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jianshenhuodong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
